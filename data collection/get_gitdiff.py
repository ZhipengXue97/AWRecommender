...
already_repo = []
commit_repo = readjson(gitdiff_log_file_path)
...
for repo_id in fixed_repo:
    ...
    for fix_pair in fix_pairs:
        ...
        cur = fix_pair[0]
        pre = fix_pair[1]
        modified = hash_commit[cur].modified_files
        for m in modified:
            diff[m.old_path] = m.diff_parsed

        commit_title = f'{repo_id}/{cur}'

        logjson(commit_title , diff, gitdiff_log_file_path)
